...
The bridge stores one queue per Studio session. `exec`, `read`, `export`, `import-asset`, and `import-image` enqueue one plugin command and wait for its result. `export` returns file payloads that the CLI writes to disk after validating every relative path. `import-asset` parses local OBJ/STL/glTF/GLB geometry in the CLI, or asks headless Blender to convert other supported DCC formats to GLB, then sends triangle mesh payloads to Studio. The plugin turns those payloads into welded MeshParts through `EditableMesh`. `import-image` decodes local PNG files in the CLI, sends RGBA pixels to Studio, and the plugin turns them into `EditableImage`-backed `ImageLabel` or `ImageButton` UI. `transfer` enqueues a `serialize` command on the source Studio, then a `deserialize` command on the target Studio with the returned blob.
